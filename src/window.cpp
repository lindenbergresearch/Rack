...
Window::Window() {
    ...
    if (!vg) {
        osdialog_message(
                OSDIALOG_ERROR, OSDIALOG_OK,
                "Could not initialize NanoVG. Does your graphics card support OpenGL 2.0 or greater? If so, make sure you have the latest graphics drivers installed.");
        exit(1);
    }

    // Load default Blendish font
    uiFont = loadFont(asset::system("res/fonts/Asimovt.ttf"));
...
void Window::run() {
    frame = 0;
    frameTime = 1. / settings::frameRateLimit;

    INFO("GPU: v-sync: %s | rate: %3.1f fps", settings::frameRateSync ? "yes" : "no", settings::frameRateLimit);

    ftime = new timer::Stopwatch(1. / settings::frameRateLimit, 3);
    stime = new timer::Stopwatch(1. / settings::frameRateLimit, 3);
    rtime = new timer::Stopwatch(1. / settings::frameRateLimit, 3);
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        if (visible) {
            ...
            if (settings::cpuMeter) {
                bndMenuBackground(args.vg, winWidth-205, 17, 190, 50, BND_CORNER_NONE);

                auto statisticsText = string::f("%3.1f\n%0003.1fms %003.1f%%\n%0003.1fms %003.1f%% ",
                                                fps,
                                                stime->elapsed * 10e3,
                                                stime->percent(),
                                                rtime->elapsed * 10e3,
                ...
                bndLabel(vg, winWidth - 155, 20, INFINITY, INFINITY, -1, statisticsText.c_str());
                bndLabel(vg, winWidth - 200, 20, INFINITY, INFINITY, -1, "fps\nstep\ndraw");
        ...
        double waitDuration = frameTime - ftime->laptime();
        if (waitDuration > 0) {
        ...
        if (frame % (int) (settings::frameRateLimit * 15) == 0) {
            DEBUG("fps(%3.1f)  step(%.2fms %.2f%%) draw(%.2fms %.2f%%)", fps,
                  stime->elapsed * 10e3,
                  stime->percent(),
                  rtime->elapsed * 10e3,
                  rtime->percent());
...
bool Window::isFrameOverdue() {
    if (settings::frameRateLimit == 0.0)        return false;

    return ftime->percent() > 100.;
}
