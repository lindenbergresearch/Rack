...
FramebufferWidget::FramebufferWidget() {
    oversample = settings::fbOversample;
}
...
void FramebufferWidget::step() {
    ...
    // dirty hack: invalidate fb else drawing results in occasionally raising double sized results!
    fbSize = math::Vec(1, 1);
    ...
    math::Vec min = localBox.getTopLeft().mult(fbScale);
    math::Vec max = localBox.getBottomRight().mult(fbScale);
    ...
    if (oversample != 1.0) {
        auto newFb = nvgluCreateFramebuffer(vg, fbSize.x, fbSize.y, 0);
        ...
        auto paint = nvgImagePattern(vg, 0.0, 0.0, fbSize.x, fbSize.y, 0.0, fb->image, 1.0);
...
void FramebufferWidget::draw(const DrawArgs &args) {
    ...
    if (bypass || args.fb) {
        Widget::draw(args);
        //  DEBUG("%p FramebufferWidget::draw", this);
        return;
    }
    ...
    nvgRect(args.vg, offset.x + fbBox.pos.x, offset.y + fbBox.pos.y, fbBox.size.x * scaleRatio.x, fbBox.size.y * scaleRatio.y);
    NVGpaint paint = nvgImagePattern(args.vg, offset.x + fbBox.pos.x, offset.y + fbBox.pos.y, fbBox.size.x * scaleRatio.x, fbBox.size.y * scaleRatio.y, 0.0, fb->image, 1.0);
